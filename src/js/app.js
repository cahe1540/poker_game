...
class card {
  ...
  setRank() {
    ...
    //generate values
    if (this.number >= 2 && this.number <= 13) this.rank = this.number;
    else if (this.number == 1) this.rank = 14;
  }
...
class player {
  ...
  isFlush() {
    if (
      this.hand[0].getcardSuit() == this.hand[1].getcardSuit() &&
      this.hand[1].getcardSuit() == this.hand[2].getcardSuit() &&
      this.hand[2].getcardSuit() == this.hand[3].getcardSuit() &&
      this.hand[3].getcardSuit() == this.hand[4].getcardSuit()
    ) {
      this.handValue[0] = 5;
  ...
  isRoyalFlush() {
    ...
    if (this.handValue[0] == 5) {
      ...
      for (let i = 0; i < 5; i++) {
        if (this.hand[i].getRank() == 14) hasValues[0] = true;
        else if (this.hand[i].getRank() == 10) hasValues[1] = true;
        else if (this.hand[i].getRank() == 11) hasValues[2] = true;
        else if (this.hand[i].getRank() == 12) hasValues[3] = true;
        else if (this.hand[i].getRank() == 13) hasValues[4] = true;
      }
    }
  ...
  displayHandValue() {
    switch (this.handValue[0]) {
      case 0:
        return (
          "Nothing was found, the largest card in hand is " + this.handValue[1]
        );
      case 1:
        return "Hand contains 1 pair, value of pair is " + this.handValue[1];
      case 2:
        return (
          "Hand contains 2 pairs, Larger pair value is " +
          this.handValue[1] +
          " Smaller pair value is " +
          this.handValue[2]
        );
      case 3:
        return (
          "Hand contains three of a kind, three of a kind value is " +
          this.handValue[1]
        );
      case 4:
        return (
          "Hand contains a straight, largest number in straight is is " +
          this.handValue[1]
        );
      case 5:
        return (
          "Hand contains a flush, larger value in flush is " + this.handValue[1]
        );
      case 6:
        return (
          "Hand contains a full house, three of kind has value of " +
          this.handValue[1] +
          "Pair has value of " +
          this.handValue[2]
        );
      case 7:
        return (
          "Hand contains a four of a kind, value of 4 of a kind is " +
          this.handValue[1]
        );
      case 8:
        return (
          "Hand contains a straight flush, highest value in flush is " +
          this.handValue[1]
        );
      case 9:
        return "Hand contains a royal flush";
    }
    return " ";
  }
}
...
class GameCtrl {
    ...
    //if they tie..
      ...
      if (temp1[0] == 2) {
        ...
        //if it's a tie, compare with smaller pairs in each hand
          ...
          if (temp1[2] > temp2[2]) return 1;
          else if (temp2[2] > temp1[2]) return 0;
          else if (temp2[2] == temp1[2]) return 2;
        }
      }
      ...
      if (temp1[0] == 3) {
        ...
        //if it's a tie still
        else if (temp2[1] == temp1[1]) return 2; //for now, just declare hand a tie... need to add method to check the remaining two card sizes
      }
      ...
      if (temp1[0] == 4) {
        ...
        //if it's a tie still
        else if (temp2[1] == temp1[1]) return 2; //still a tie
      }
  ...
  updateCredits() {
    ...
    if (this.compareHands() === 1) {
      switch (this.player.handValue[0]) {
        case 1:
          this.player.addTokens(6);
          return 6;

        case 2:
          this.player.addTokens(12);
          return 12;

        case 3:
          this.player.addTokens(24);
          return 24;

        case 4:
          this.player.addTokens(48);
          return 48;

        case 5:
          this.player.addTokens(102);
          return 102;

        case 6:
          this.player.addTokens(205);
          return 205;

        case 7:
          this.player.addTokens(417);
          return 417;

        case 8:
          this.player.addTokens(837);
          return 837;

        case 9:
          this.player.addTokens(1500);
          return 1500;
  ...
  toggle(index) {
    if (this.cardsToChange[index] === 1) this.cardsToChange[index] = 0;
    else this.cardsToChange[index] = 1;
  }

  resetAllCardsToChange() {
    for (var i = 0; i < 5; i++) this.cardsToChange[i] = 0;
  }
}
...
class UICtrl {
  ...
  displayHand(player) {
    var dom;
    if (player.name === "dealer") dom = ".Dcard";
    else if (player.name === "player") dom = ".Pcard";

    let time = 600;
    for (let i = 1; i <= 5; i++) {
      setTimeout(() => {
        this.displayACard(dom, i);
      }, time);
      time += 600;
    }
  }
  ...
  displayACard(playerType, cardNum) {
    if (playerType === ".Dcard")
      document.querySelector(playerType + "-" + cardNum).src =
        this.game.dealer.hand[cardNum - 1].getImageFileName();
    else if (playerType === ".Pcard")
      document.querySelector(playerType + "-" + cardNum).src =
        this.game.player.hand[cardNum - 1].getImageFileName();
  }
  ...
  static showElement(el) {
    ...
    else document.querySelector(el).style.display = disable;
  }
  ...
  updateRoundResult(win) {
    ...
    if (win === 1) result = "Player 1 Wins!";
    else if (win === 2) result = "Tie!";
    else if (win === 0) result = "Dealer Wins!";
  ...
  showScoreChange(winner, scoreChange) {
    ...
    else document.querySelector(this.scoreChange).innerHTML = "--";
  }
  ...
  ToggleBorder(el, val) {
    ...
    else document.querySelector("." + el).style.border = "none";
  }
...
class Ctrl {
  ...
  setUpEventListeners() {
    ...
    document
      ...
      .addEventListener("click", this.newGame.bind(this));
    ...
    //trade cards button
    document
      .querySelector(".trade-cards")
      .addEventListener("click", this.tradeCards.bind(this));
    ...
    for (var i = 1; i <= 5; i++)
      document
        .querySelector(".Pcard-" + i)
        .addEventListener("click", this.cardEvent.bind(this));
  }
  ...
  go() {
    ...
    if (this.UI.game.player.tokens > 0) {
      if (!this.UI.game.endTurn) {
        ...
        this.UI.displayHandValues(
          this.UI.game.dealer.displayHandValue(),
          this.UI.game.player.displayHandValue()
        );
  ...
  tradeCards() {
    if (this.UI.game.player.tokens > 0) {
      ...
      if (this.UI.game.handChangeCounter === 0) {
        ...
        this.UI.showMessageToPlayer(
          "Click cards that you want to swap, double click to unselect. <br>" +
